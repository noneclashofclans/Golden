...
import { User, Mail, Lock, Phone, Sparkles, Terminal, AlertCircle, CheckCircle, Loader } from 'lucide-react';
  ...
  return (
    <div>
      <motion.div className="relative group" animate={focused ? { scale: 1.02 } : { scale: 1 }} transition={{ duration: 0.2 }}>
        ...
        />
        <motion.div
          className={`absolute bottom-0 left-4 right-4 h-px rounded-full ${error ? 'bg-red-500' : 'bg-gradient-to-r from-yellow-500 to-orange-500'}`}
...
const RegisterPage = () => {
  ...
  const validate = () => {
    ...
    else if (form.username.trim().length < 2) e.username = 'At least 2 characters';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('https://golden-back-pftn.onrender.com/api/auth/register', {
        method: 'POST',
      ...
      // ── Success: account created, send to login to get JWT ──
      setSuccess(true);
      showToast(`Welcome, ${data.username}! Account created. Redirecting to login...`, 'success');
        ...
        >
          ...
          <div className="relative bg-white/[0.04] border border-white/10 backdrop-blur-2xl rounded-[2.5rem] p-10 shadow-[0_0_120px_rgba(0,0,0,0.95)]">
            ...
            <div className="text-center mb-8">
              <motion.div
                className={`w-fit p-4 rounded-2xl mx-auto mb-6 border transition-colors ${success ? 'bg-green-500/10 border-green-500/30 shadow-[0_0_30px_rgba(34,197,94,0.2)]' : 'bg-yellow-500/10 border-yellow-500/30 shadow-[0_0_30px_rgba(234,179,8,0.2)]'}`}
                animate={{ y: [0, -6, 0] }} transition={{ duration: 2.5, repeat: Infinity, ease: 'easeInOut' }}
                whileHover={{ rotate: 180, scale: 1.1 }}
              >
                {success ? <CheckCircle size={32} className="text-green-500" /> : <Terminal size={32} className="text-yellow-500" />}
              </motion.div>

              <motion.h2 className="text-5xl font-black mb-2 tracking-tight" style={{ fontFamily: "'Bebas Neue', sans-serif" }}
                initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                {success
                  ? <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-emerald-500">SUCCESS!</span>
              ...
              <motion.p className="text-slate-400 text-[10px] uppercase tracking-[0.3em] font-bold"
                ...
                {success ? 'Account Created · Redirecting to Login' : 'New Developer Setup'}
              </motion.p>
            </div>
            ...
            <AnimatePresence mode="wait">
              {!success && (
                <motion.form key="register-form" exit={{ opacity: 0, y: -10 }} className="space-y-4" onSubmit={handleSubmit}>
                  ...
                  <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.56 }}>
                    <InputField icon={<Phone size={18} />} type="tel" placeholder="PHONE_NUMBER"
                      value={form.phone} onChange={handleChange('phone')} error={errors.phone} />
                  </motion.div>

                  <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.64 }}>
                  ...
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.75 }} className="pt-2">
                    <motion.button type="submit" disabled={loading}
                      whileHover={!loading ? { scale: 1.03, boxShadow: '0 0 60px rgba(234,179,8,0.45)' } : {}}
                      whileTap={!loading ? { scale: 0.97 } : {}}
                      className="w-full bg-gradient-to-r from-yellow-500 to-orange-500 text-black font-black py-4 rounded-xl flex items-center justify-center gap-2 uppercase tracking-widest text-xs shadow-[0_0_40px_rgba(234,179,8,0.25)] relative overflow-hidden group disabled:opacity-60 disabled:cursor-not-allowed"
                    >
                      ...
                      <motion.span className="relative z-10 flex items-center gap-2"
                        ...
                        {loading ? (
                          <><Loader size={15} className="animate-spin" /> Initializing Account...</>
                        ) : (
                          <>
                            Execute Registration
                            <motion.span animate={{ rotate: [0, 20, -20, 0], scale: [1, 1.2, 1] }} transition={{ duration: 2, repeat: Infinity }}>
                              <Sparkles size={15} />
                            </motion.span>
                          </>
                        )}
            ...
            <motion.p className="text-center mt-8 text-slate-400 text-[10px] tracking-widest uppercase"
